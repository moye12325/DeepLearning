...
# 输入部分包含:源文本嵌入层及其位置编码器、目标文本嵌入层及其位置编码器
# 输出部分包含:线性层、softmax处理器

# 编码器部分:由N个编码器层堆叠而成、每个编码器层由两个子层连接结构组成、
...
# 输入部分----------源文本嵌入层及其位置编码器、目标文本嵌入层及其位置编码器------------------
# 源文本嵌入层及其位置编码器
# 目标文本嵌入层及其位置编码器

# 1.1文本嵌入层：文本中的词汇转为向量表示
import torch
...
# 1.2位置编码器：词汇位置不同可能会产生不同语义，Transformer的编码器结构中, 并没有针对词汇位置信息的处理
...
# 至于是0位置被遮掩还是1位置被遮掩可以自定义，
# 因此它的作用就是让另外一个张量中的一些数值被遮掩，也可以说被替换,
# 它的表现形式是一个张量.
...
# 2.2注意力机制：
# 注意力机制是注意力计算规则能够应用的深度学习网络的载体,
# 除了注意力计算规则外, 还包括一些必要的全连接层以及相关张量处理,
# 使其与应用网络融为一体. 使用自注意力计算规则的注意力机制称为自注意力机制.
...
# 2.3多头注意力机制：
# 从多头注意力的结构图中，貌似这个所谓的多个头就是指多组线性变换层，
# 其实并不是，我只有使用了一组线性变化层，即三个变换张量对Q，K，V分别进行线性变换，
# 这些变换不会改变原有张量的尺寸，因此每个变换矩阵都是方阵，
# 得到输出结果后，多头的作用才开始显现，
# 每个头开始从词义层面分割输出的张量，
# 也就是每个头都想获得一组Q，K，V进行注意力机制的计算，
# 但是句子中的每个词的表示只获得一部分，也就是只分割了最后一维的词嵌入向量.
# 这就是所谓的多头，将每个头的获得的输入送到注意力机制中, 就形成多头注意力机制.
